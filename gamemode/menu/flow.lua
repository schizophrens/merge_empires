...
local function bindPanelCallbacks(panel)
    ...
    panel:AddFunction("CineEvent", "PartyInviteAnswer", function(ok)
        net.Start("ME_Party_InviteAnswer")
        net.WriteBool(tostring(ok or "0") == "1")
        net.SendToServer()
    end)
...
net.Receive("ME_Party_InviteReq", function()
    net.ReadString()
    local name = net.ReadString()
    if not IsValid(S.panel) then return end
    S.panel:RunJavascript("try{CineAPI.inviteRequest(" .. jsq(name) .. ")}catch(e){}")
end)
